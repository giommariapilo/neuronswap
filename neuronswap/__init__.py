...
from .optimizermatrixswap import swap_inverted as moswapinv
from .optimizerindexswap import swap as ioswap
from .optimizerindexswap import swap_inverted as ioswapinv
from .matrixswap import swap as mswap
from .matrixswap import swap_inverted as mswapinv
from .indexswap import swap as iswap
from .indexswap import swap_inverted as iswapinv
...
def permute_inverted(layers_list: list[nn.Module], permutations: dict[str, torch.Tensor | list[int]], skip_connections: list[str] = []):
  '''This function takes as inputs the list of layers of a model, a dictionary containing the 
  permutation matrix associated to each layer or the indexes of the neurons to be moved at the top 
  of the layer, and an optional list of skip connections. It then calls one of the two 
  implementations of the swap method to swap the neurons. The last layer won't be permuted as it 
  ...
  permute_function = mswapinv if isinstance(list(permutations.values())[0], torch.Tensor) and len(list(permutations.values())[0].shape) > 1 else iswapinv
  return permute_function(layers_list, permutations, skip_connections)
...
def permute_optimizer_inverted(layers_list: list[nn.Module], permutations: dict[str, torch.Tensor], model: nn.Module, optimizer: torch.optim.Optimizer,skip_connections: list[str] = []):
  ''''''
  type_check = None
  shape_check = 0
  for permutation in permutations.values():
    ...
    if type_check != None:
      ...
      if type(permutation) == torch.Tensor:
        shape_check = len(permutation.shape)
  permute_function = moswapinv if isinstance(list(permutations.values())[0], torch.Tensor) and len(list(permutations.values())[0].shape) > 1 else ioswapinv
  return permute_function(layers_list, permutations, model, optimizer, skip_connections)
