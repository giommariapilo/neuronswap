...
def swap_layer(layer: nn.Module, permutation_matrix: torch.Tensor):
  ...
  It then permutes the layer swapping the rows. It is implemented as a matrix multiplication.
  ...
  if len(layer.weight.data.shape) != len(permutation_matrix.shape):
    ...
    layer.weight.data = torch.reshape(permuted_weight, weight_shape)
  else:
...
def swap_input_channels(layer: nn.Module, previous_layer: nn.Module, permutation_matrix: torch.Tensor):
  ...
  if len(layer.weight.data.shape) != len(permutation_matrix.shape):
    ...
    for i in range(reshaped_weight.shape[-1]):
      permuted_weights[:,:,i] = torch.matmul(reshaped_weight[:,:,i], permutation_matrix)

    layer.weight.data = permuted_weights.reshape(weight_dim)
...
def swap(layers_list: list[nn.Module], permutations: dict[str, torch.Tensor], skip_connections: list[str] = []):
  ...
  It then sequentially permutes each layer, also permutating the input channels. The 
  last layer won't be permuted as it will change the output of the network. If it 
  receives a list of skip connections, those layers are not permuted as permutation 
